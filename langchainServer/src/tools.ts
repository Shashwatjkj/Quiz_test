...
export const sendEmailTool = tool(
  async ({ recipient, subject, body, cc, bcc }) => {
    return await sendEmail(recipient, subject, body, cc, bcc);
  },
  {
    name: "send_email",
    description:
      "Send an email to a recipient. Use this when you need to send approval requests or communicate with people.",
    schema: z.object({
      recipient: z
        .string()
        .email()
...
// export const checkEmailsTool = tool(
//   async ({ unreadOnly }) => {
...
//     name: "check_emails",
//     description:
//       "Check for new email responses in the inbox.",
//     schema: z.object({
//       unreadOnly: z
//         .boolean()
//         .default(true)
...
//     return await processEmailResponse(
//       emailId,
//       action as "approve" | "reject" | "escalate" | "save"
//     );
//   },
//   {
//     name: "process_email",
//     description:
//       "Process an email response by taking an action (approve, reject, escalate, or save).",
//     schema: z.object({
